...
df_rules <- df_rules %>%
  ...
  select(SquatInc1_class, SquatInc2_class, BenchInc1_class, BenchInc2_class, DeadliftInc1_class, DeadliftInc2_class, Age_class, Equipment, BodyWeight_class, Sex, Event, Equipment, Place, Tested, Place) %>%
  mutate(across(1:13, ~factor(.x)))



df_apriori <- transactions(df_rules)

rules_apriori <- apriori(df_apriori, parameter = list(supp = 0.1, conf = 0.80) , minlen=2, maxlen=2)
