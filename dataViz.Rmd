...
###
plot(x=cols_zero_var$Age,y=cols_zero_var$Goodlift)
plot(x=cols_zero_var$Age,y=cols_zero_var$BodyweightKg)
plot(x=cols_zero_var$Squat,y=cols_zero_var$Deadlift)

pairs(~Age+Squat+Bench+Deadlift,data=df1)
pairs(~Age+Squat+SquatInc1,data=df1)
...
univariate_vals_long %>%
  filter(metric %in% c("kurtosis", "skewness")) %>%
...
```{r}
df %>%
  select(Age, BodyweightKg, Goodlift, Squat, Bench, Deadlift, Sex) %>%
  pivot_longer(!Sex) %>%
  ggplot(aes(x = value, fill = Sex)) + geom_histogram(alpha = 4/10, position = "identity") + facet_wrap(~name, scales = "free")

summary(cols_zero_var)
...
groups <- list(
  ...
  # All attempts absolute
  # "Lift tried"=list("s",
  #   c("WeightTriedSquat1Kg", "WeightTriedSquat2Kg", "WeightTriedSquat3Kg",
  #   "WeightTriedBench1Kg", "WeightTriedBench2Kg", "WeightTriedBench3Kg",
  #   "WeightTriedDeadlift1Kg", "WeightTriedDeadlift2Kg", "WeightTriedDeadlift3Kg")),
  # "Lift quality"=list("n",
  #   c("LiftedSquat1Kg?", "LiftedSquat2Kg?", "LiftedSquat3Kg?",
  #   "LiftedBench1Kg?", "LiftedBench2Kg?", "LiftedBench3Kg?",
  #   "LiftedDeadlift1Kg?", "LiftedDeadlift2Kg?", "LiftedDeadlift3Kg?")),
...
```{r}
for (i in 1:4) {
  p <- fviz_contrib(res.mfa, "group", axes = i)
  print(p)
}
```
...
for(axes in list(c(1,2), c(2, 3), c(3, 4))) {
  p <- fviz_mfa_var(
    res.mfa, choice = "group", axes = axes,
...
```{r}
for (i in 1:4) {
  p <- fviz_contrib(res.mfa, choice = "quanti.var", axes = i, top = 10)
  print(p)
  p <- fviz_contrib(res.mfa, choice = "quali.var", axes = i, top = 10)
  print(p)
}
```
...
for(axes in list(c(1, 2), c(2, 3), c(3, 4))) {
  p <- fviz_mfa_var(
    res.mfa, "quanti.var", axes = axes,
    select.var = list(cos2 = 20),
    col.var = "contrib", gradient.cols = grad_col, repel = TRUE
  )
  print(p)
  p <- fviz_mfa_var(
    res.mfa, "quali.var", axes = axes,
    select.var = list(cos2 = 20),
    col.var = "contrib", gradient.cols = grad_col, repel = TRUE
  )
  print(p)
...
```{r}
for(axes in list(c(1,2), c(2, 3), c(3, 4))) {
  p <- fviz_mfa_axes(res.mfa, axes = axes, repel=T)
  print(p)
}
```
...
```{r}
# Check correlation between Dim 2 and Place
boxplot(res.mfa$ind$coord[,2]~df_mfa$Place, main = "MFA Dim 1 vs Place", xlab = "Place", ylab = "Dim 1")
```
...
```{r}
# Check individuals projections to a combined vector of fails
...
colvar <- as.integer(df_mfa[rownames(res.mfa$ind$coord),][smpl,]$Place)
col <- hcl.colors(length(levels(df_mfa$Place)), palette = "viridis")
scatter3D(px, py, pz, col=col, colvar = colvar, pch = 20, cex = 0.5, bty = "n")
...
ay <- c(0, 5, 0)
az <- c(0, 0, 5)
arrows3D(zeros, zeros, zeros, ax, ay, az, bty = "n", lwd = 2, col = "red", add=T)
text3D(ax, ay, az, c("D1", "D2", "D3"), bty = "n", col = "red", add=T)
...
arrows3D(zeros, zeros, zeros, ax, ay, az, bty = "n", lwd = 2, add=T)
text3D(ax, ay, az, rownames(res.mfa$quali.var$coord), add=T, bty = "n")

# Quantitative variables
zeros <- rep(0, nrow(res.mfa$quanti.var$coord))
ax <- res.mfa$quanti.var$coord[,ds[1]]
ay <- res.mfa$quanti.var$coord[,ds[2]]
az <- res.mfa$quanti.var$coord[,ds[3]]
arrows3D(zeros, zeros, zeros, ax, ay, az, bty = "n", lwd = 2, add=T)
text3D(ax, ay, az, rownames(res.mfa$quanti.var$coord), add=T, bty = "n")
